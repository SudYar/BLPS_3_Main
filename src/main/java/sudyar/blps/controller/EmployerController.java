...
import sudyar.blps.dto.request.ChosenRequest;
...
public class EmployerController {
	...
	public InfoResponse deleteById(@RequestBody Integer idOrdering) {
		...
		if (order.isPresent()) {
			...
			if (login.equals(ordering.getOwnerLogin())) {
				...
				return new InfoResponse("Удален заказ: " + ordering, 0);
	...
	public InfoResponse chooseExecutor(@RequestBody ChosenRequest chosenRequest) {
		int idOrder = chosenRequest.getIdOrder();
		String executor = chosenRequest.getExecutor();
	...
	public InfoResponse finishOrder(@RequestBody FeedbackRequest feedbackRequest) {
		...
		if (starsForFeedback < 0 || starsForFeedback > 10)
			return new InfoResponse("Количество звезд должно быть целым в пределах [0;10]", 1);
		Optional<Ordering> optionalOrdering = orderService.getById(idOrder);
		if (optionalOrdering.isPresent()) {
			Ordering order = optionalOrdering.get();
			String employer = SecurityContextHolder.getContext().getAuthentication().getName();
